...
//---------------------------------------------------------------------------
int printcar(int, BOOL ,int,AnsiString,AnsiString,AnsiString,AnsiString,AnsiString,float,float,float,float,float,float,float,float,float,float,float,float,float,float,float,float,float,float,float) ;
...
float sn=0; // стоимость в в ыходные
float chn=0    ; // часы в выходные дни
float ccn=0; // ЦЕНА ЗА вых  дни.
float chas=0,km=0;
int n=1; // номер п/п
...
AnsiString Vyhodnoy[50];  // строка с выходными днями
int count_vyh=0;
AnsiString Night[50];  // строка с выходными днями
int count_night=0;
...
return;
  ...
  float           differ=0,dif_cc=0,dif_ccn=0;
  BOOL           vidtr;
  ...
  if (f1== Query1->FieldByName("District")->AsString  )
  {
   if (f2== Query1->FieldByName("Object")->AsString  )
   {
    if (vidtr == Query1->FieldByName("Vidtr")->AsBoolean  )
    {
     ...
     {
      dif_cc= Query1->FieldByName("Za1_Chv")->AsFloat;
      if ( cc!= dif_cc)      // если   другой тариф в выхлдные дни вывод старых данных и сброс счетчиков
      {
      ...
      dif_ccn= Query1->FieldByName("Za1_Chn")->AsFloat;
      if ( ccn!= dif_ccn)      // если   другой тариф в выхлдные дни вывод старых данных и сброс счетчиков
      {
       if ((ccn*chn)!=0)    // вывод вых дней
       {
        Night[count_night]=".\t З/пл. в ночное время \t прц  \t ч \t" + FloatToStrF(ccn,ffFixed, 10, 2) + "\t"  + FloatToStrF(chn, ffFixed, 10, 2) + "\t" +  FloatToStrF(ccn*chn,ffFixed, 10, 2);
        //Form8->Memo1->Lines->Add(IntToStr(n) +".\t З/пл. в вых день \t прц  \t ч \t" + IntToStr(cc) + "\t"  + FloatToStrF(ch, ffGeneral, 4, 2) + "\t" +  IntToStr(int(cc*ch)));
        //  n++;
        sn=sn+ ccn*chn;
        count_night++;
     ...
     else // если изменилась марка номер прицеп тариф за км
     {      /// вывыод информации о транспорте
      n= printcar( n , vidtr , vid_izm, f1,f2,f3,f4, f5, f6, f7, cc, ch,ccn,chn, km, chas, s1_m, s2_m,sum_mch,sum_tkm,sum_t,ed_mch,ed_t,ed_tkm,col_mch,col_t,col_tkm)   ;
      if(count_vyh)
      {
      ...
      if(count_night)
      {
       for (int h=0;h<count_night; h++)
       {
        Memo1->Lines->Add(IntToStr(n) + Night[h]);
        n++;
       }
       count_night=0;
      }

      s1_v+=s1_m + s2_m + cc*ch + ccn*chn + sum_tkm + sum_t + sum_mch+sv+sn;    // ЕСЛИ МАШИНА ДРУГАЯ
      s1_m=s2_m=cc=ch=ccn=chn=chas=km=0;
      sum_mch=sum_tkm=sum_t=ed_mch=ed_t=ed_tkm=col_mch=col_t=col_tkm=sv=sn=0;
      f3= Query1->FieldByName("Marka")->AsString;
      f4= Query1->FieldByName("Govnumber")->AsString;
      f5= Query1->FieldByName("Pricep")->AsString ;
      f6=f7=0;
...
float cc=0,dif_cc=0; // ЦЕНА ЗА вых  дни.
float chn=0;   // часы ЗА ночь
float ccn=0,dif_ccn=0; // ЦЕНА ЗА ночь
int n=1; // номер п/п
float sv=0;// сумма за вых дни
float sn=0;// сумма за ночные
float chas =0;
float cost_chas=0;// считаем часы и тарифы

AnsiString Vyhodnoy[100];  // строка с выходными днями
int count_vyh=0;
AnsiString Night[100];  // строка с ночными
int count_night=0;
...
ccn= Query2->FieldByName("Za1_Chn")->AsFloat;
...
while ( !Query2->Eof)
{
 ...
 {
  dif_cc= Query2->FieldByName("Za1_Chv")->AsFloat;
  if ( cc!= dif_cc)      // если   другой тариф в выхлдные дни вывод старых данных и сброс счетчиков
  {
   if ((cc*ch)!=0)    // вывод вых дней
   {
  ...
  dif_ccn=Query2->FieldByName("Za1_Chn")->AsFloat;
  if ( ccn!= dif_ccn)      // если   другой тариф в выхлдные дни вывод старых данных и сброс счетчиков
  {
   if ((ccn*chn)!=0)    // вывод вых дней
   {
  ...
  if(Query2->FieldByName("Ch_n")->AsFloat!=0)
  {
   chn= chn+Query2->FieldByName("Ch_n")->AsFloat;
   ccn= Query2->FieldByName("Za1_Chn")->AsFloat;
  }
 }
 else
 {  // если механизмы вывод без номера
  ...
  if ((ccn*chn)!=0)    // вывод нчного времени
  {
   Memo1->Lines->Add(IntToStr(n) +".\t З/пл. в ночное время \t прц  \t ч \t" + FloatToStrF(ccn,ffFixed,10,2) + "\t"  + FloatToStrF(chn, ffFixed, 10, 2) + "\t" +  FloatToStrF(ccn*chn,ffFixed,10,2));
   n++;
   if(count_night)
   {
    for (int h=0;h<count_night; h++)
  ...
  s1_v+=s1_m + s2_m + cc*ch +sv+sn;    // ЕСЛИ МАШИНА ДРУГАЯ
  s1_m=s2_m=cc=ch=chn=ccn=chas=cost_chas=sv=sn=0;
  f3= Query2->FieldByName("Marka")->AsString;
  f4= Query2->FieldByName("Govnumber")->AsString;
  f5= Query2->FieldByName("Pricep")->AsString ;
  ...
  cc=Query2->FieldByName("Za1_Chv")->AsFloat;
  chn=Query2->FieldByName("Ch_n")->AsFloat;
  ccn=Query2->FieldByName("Za1_Chn")->AsFloat;
 }
 Query2->Next()  ;
}
...
if ((ccn*chn)!=0)    // вывод ночных дней
{
 Memo1->Lines->Add(IntToStr(n) +".\t З/пл. в ночное время \t прц  \t ч \t" + FloatToStrF(ccn, ffFixed,10,2) + "\t"  + FloatToStrF(chn, ffFixed, 10, 2) + "\t" +  FloatToStrF(ccn*chn,ffFixed,10,2));
 n++;
 if(count_night)
...
float ch=0;    // часы ЗА вых  дни.
float cc=0; // ЦЕНА ЗА вых  дни.
float sn=0; // сумма за выходные дни
float chn=0;    // часы ЗА вых  дни.
float ccn=0; // ЦЕНА ЗА вых  дни.
float chas=0,km=0;
int n=1; // номер п/п
int vid_izm; /// Наимнование (т,км,ч,м/ч)
float dif_cc=0,dif_ccn=0;
float sum_tkm=0,sum_t=0,sum_mch=0;
float col_tkm=0,col_t=0,col_mch=0;
  ...
  if (f2== Query1->FieldByName("Object")->AsString  )
  {
   ...
   { // считаем для вида транспорта
    ...
    { // вывыод информации о транспорте
     s1_v+=s1_m + s2_m  + cc*ch + ccn*chn + sum_tkm + sum_t + sum_mch+sv+sn;    // ЕСЛИ МАШИНА ДРУГАЯ
     s1_m=s2_m=cc=ch=ccn=chn=chas=km=0;
     sum_mch=sum_tkm=sum_t=col_mch=col_t=col_tkm=sv=sn=0;
     //s1_ob+=s1_v;    // ЕСЛИ транспорт другой
     // s1_v=0;
     f3= Query1->FieldByName("Marka")->AsString;
     f4= Query1->FieldByName("Govnumber")->AsString;
     ...
     ch=Query1->FieldByName("Ch_v")->AsFloat;
     cc=Query1->FieldByName("Za1_Chv")->AsFloat;
     chn=Query1->FieldByName("Ch_n")->AsFloat;
     ccn=Query1->FieldByName("Za1_Chn")->AsFloat;
     if(vid_izm==1)
     {
      s1_m=  Query1->FieldByName("Cost1")->AsFloat;
...
{
 // вывыод информации о транспорте
 s1_v+=s1_m + s2_m + cc*ch + ccn*chn + sum_tkm + sum_t + sum_mch;    // ЕСЛИ МАШИНА ДРУГАЯ
 s1_m=s2_m=cc=ch=chn=ccn=chas=km=0;
 sum_mch=sum_tkm=sum_t=col_mch=col_t=col_tkm=sv=sn=0;
 n=1;
 s1_ob+=s1_v;    // ЕСЛИ транспорт другой
 s1_v=0;
...
int printcar(int n,BOOL vidtr , int vid_izm, AnsiString f1,AnsiString f2, AnsiString f3,AnsiString f4,AnsiString f5,float f6,float f7,float cc,float ch,float ccn,float chn,float km,float chas,float s1_m,float s2_m,float sum_mch,float sum_tkm,float sum_t,float ed_mch,float ed_t,float ed_tkm,float col_mch,float col_t,float col_tkm)
{
 ...
 if ((cc*ch)!=0)    // вывод вых дней
 {
  ...
  if ((ccn*chn)!=0)    // вывод вых дней
  {
   Form8->Memo1->Lines->Add(IntToStr(n) +".\t З/пл. в ночное время \t прц  \t ч \t" + FloatToStrF(ccn,ffFixed,10,2) + "\t"  + FloatToStrF(chn,ffFixed,10,2) + "\t" +  FloatToStrF(ccn*chn,ffFixed,10,2));
   n++;
  }
   return n;
}
